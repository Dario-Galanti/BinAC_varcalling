...
#fin_str=$(ls ${inDir}/*v3_Ta_variants.vcf.gz | sort -V | sed ':a;N;$!ba;s/\n/ -I /g')
#fin_str=$(zcat ${inDir}/Scaffold_4569_v3_Ta_variants.vcf.gz | grep 'contig=<ID' | awk -v inDir="${inDir}" -F'[=,]' '{print inDir"/"$3"_v3_Ta_variants.vcf.gz"}' | sed ':a;N;$!ba;s/\n/ -I /g')
used_scaff=( $(ls ${inDir}/*v3_Ta_variants.vcf.gz | sort -V ) )
ordered_scaff=( $(zcat ${inDir}/Scaffold_4569_v3_Ta_variants.vcf.gz | grep 'contig=<ID' | awk -v inDir="${inDir}" -F'[=,]' '{print inDir"/"$3"_v3_Ta_variants.vcf.gz"}') )
